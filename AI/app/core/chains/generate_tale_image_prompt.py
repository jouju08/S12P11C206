...
Context(상황):
- 동화의 제목을 기반으로 디퓨전 모델에서 사용할 prompt와 negative prompt를 작성하려고 합니다.
- 목표는 클래식 동화의 내용과 분위기를 잘 반영하여, 메인 삽화의 스타일과 디테일을 효과적으로 증강시키는 것입니다.

Input Values(입력값):
- Title: 동화의 제목

Instructions(단계별 지시사항):
1. 동화의 주요 내용을 기반으로 삽화의 중심 요소(예: 캐릭터, 배경, 감정 등)를 추출합니다.
2. 동화의 등장인물, 배경, 물건등의 요소에 맞게 디퓨전 prompt를 작성합니다.
3. prompt는 구체적이고 상세한 묘사와 적합한 예술적 스타일을 포함합니다(예: 수채화 스타일, 디즈니풍, 3D 아트 등).
4. negative prompt에는 불필요하거나 의도하지 않은 결과물을 방지하기 위한 요소를 포함합니다(예: 흐릿함, 왜곡, 과도한 세부 묘사 등).
...
Negative Prompt: Blurry or distorted details, overly realistic textures, modern or futuristic elements, horror or dark themes, exaggerated aggression, stiff or unnatural poses, pixelated rendering, muted or dull colors, excessive violence or destruction
"""),
    ("human", """
Title: {title}
"""),
