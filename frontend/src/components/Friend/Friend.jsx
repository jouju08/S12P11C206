...
const FriendsPage = ({friends}) => {
  ...
  const { friendRequests, friendList, loading, error, fetchFriendRequests, 
    fetchFriendList,deleteFriend,respondToRequest,fetchSendFriendRequests,
    deleteRequest,fetchFindFriends,sendFriendRequests, fetchFindMembers,searchMembers, makeFriend } =
    useFriendStore();
  ...
  return (
    <div className="w-[496px] h-[497px] relative bg-white shadow-[4px_4px_4px_0px_rgba(0,0,0,0.10)] p-6">
      <div className="flex justify-around pb-2 mb-4">
        ...
        >
          <img className="w-8 h-7 rounded-full" src="/Common/search.png"/>
        </button>
      ...
      {activeTab === "get_requests" && (
        ...
        {friendRequests.length === 0 ? (
          <>
          <p className="text-center text-lg text-text-first font-NPSfont">받은 친구 요청이 없어요</p>
          </>
        ) : (
          <ul className="w-full">
        ...
        {sendFriendRequests.length === 0 ? (
          <p className="text-center text-lg text-text-first font-NPSfont">보낸 친구 요청이 없어요</p>
        ) : (
          <ul className="w-full">
      ...
      {activeTab === "friends" && (
        <div className="w-[464px] h-[406px] overflow-y-auto bg-white rounded-2xl border border-[#9f9f9f] flex flex-col items-center p-4">
          {friendList.length === 0 ? (<>
            ...
            <p className="text-center text-lg text-text-first font-NPSfont">친구를 찾아볼까요?</p>
          </>
            
          ) : (
            <ul className="w-full">
              {friendList.map((friend) => (
                <li key={friend.loginId} className="flex items-center justify-between w-full p-3 border-b border-[#e6e6e6]">
                  <div className="flex items-center gap-3">
                  <img className="w-12 h-12 rounded-full" 
                  src={friend.profilePic || "/Common/blank_profile.jpg"}
                  alt="profile"  />
                  
                    <span className="text-text-first text-lg font-NPSfont">{friend.nickname}</span>
                  </div>
                  <div className="flex gap-2">
                  <button
    ...
    focus:outline-none focus:ring-2 focus:ring-blue-500"/>
        ...
        {filteredMembers.length>0?(filteredMembers.map((member)=>{
             const isFriend = friendList.some(friend => friend.loginId === member.loginId);
             const isRequested=sendFriendRequests.some(friend=>friend.loginId===member.loginId);//이미 친구 신청중인 사람은 친구 신청 버튼 비활성화화
             return(
            <li
            key={member.loginId}
            className="flex items-center justify-between w-full p-3 border-b border-[#e6e6e6]"
        ...
        ):(<li className="px-4 py-4 text-text-first">닉네임을 다시 확인해주세요!</li>
        )}
          </ul>
          )}
      </div>
