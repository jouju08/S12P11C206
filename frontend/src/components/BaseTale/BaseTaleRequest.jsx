...
const BaseTaleRequest = () => {
  ...
  const fetchBaseTales = async () => {
    try {
      const response = await api.get('/parent/tale/base-tale');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (submitSelectedIntroImage() && submitSelectedTitleImage()) {
  ...
  const handleSelectTale = async (id) => {
    try {
      const response = await api.get(`/parent/tale/base-tale/${id}`);
      handleNewTale();
      if (response.data && response.data.data) {
          ...
          <form
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <label className="w-full font-medium text-gray-700">
                도입부 이미지
                {!selectedTitleImage && (
                  <span> (동화의 제목과 도입부로 표지를 생성합니다.)</span>
                )}
              </label>
            </div>
            <div className="flex-1">
...
export default BaseTaleRequest;
